...
**Input**: `specs/001-webware-tools-alignment/spec.md`, `plan.md`

Package parameters: PHP `["8.4", "8.5"]`, `run-integration: false`, codecov + infection on,
`coverage-php-version: 8.5`, MSI `95` / `95`, no DB container, test namespaces
`WebwareTest\Admin\` and `WebwareTestIntegration\Admin\`.

## Phase 1 — composer.json

- [ ] T001 `require.php` already `~8.4.1 || ~8.5.0` (no `8.6.0-dev`). Set
  `config.platform.php` to `8.4.99` in `composer.json`
- [ ] T002 Align `require-dev`: keep PHPUnit `^13.3`, add `infection/infection: ^0.34.1`,
  `phpbench/phpbench: ^1.7`, `roave/backward-compatibility-check: ^8.21.0`,
  `roave/security-advisories: dev-master`; keep `webware/coding-standard`,
  `webware/webware-tools`
- [ ] T003 Rename test autoload namespaces to `WebwareTest\Admin\` → `test/unit/` and
  `WebwareTestIntegration\Admin\` → `test/integration/` in `composer.json`; update `namespace`
  lines in `test/unit/*.php` to match
...
- [ ] T007 Test scaffolding: unit suite already populated; add one integration test in
  `test/integration/` (`WebwareTestIntegration\Admin\`) so both suites are non-empty
...
- [ ] T015 Create `.github/workflows/continuous-integration.yml` wrapper calling
  ...
  `min-msi: 15`, `min-covered-msi: 15` (raise toward 95 as the test suite matures)
