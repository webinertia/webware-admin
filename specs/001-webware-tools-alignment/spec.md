...
**Input**: User description: "Align webware-admin's CI/CD pipeline and dev tooling with the
...
consumer-side contract for webware-admin: which artifacts the package must provide, what shape
they take, and which values are package-specific parameters.
...
| Parameter | webware-admin value |
|---|---|
| `php-versions` | `["8.4", "8.5"]` |
| `require.php` | `~8.4.1 \|\| ~8.5.0` |
| `config.platform.php` | `8.4.99` |
| `run-integration` | `false` (no integration suite yet) |
| `enable-codecov` | `true` |
| `enable-infection` | `true` |
| `coverage-php-version` | `8.5` |
| `min-msi` / `min-covered-msi` | `0` / `0` (fresh import; raise toward 95 as the test suite matures) |
...
strategies), Codecov upload, and Infection mutation testing.
...
- Integration tests do not exist yet: `run-integration` is `false`; the integration leg is
  skipped until a suite exists.
