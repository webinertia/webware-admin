...
**Branch**: `specify-alignment-spec` | **Date**: 2026-08-18 | **Spec**: `specs/001-webware-tools-alignment/spec.md`

**Input**: Feature specification from `/specs/001-webware-tools-alignment/spec.md`

## Summary

Align webware-admin's CI/CD pipeline and dev-tooling configuration with the
...
**Storage**: N/A — no database. Reusable workflow DB steps skipped (`db-image` omitted).
...
test/
├── unit/                                  # WebwareTest\Admin\
└── integration/                           # WebwareTestIntegration\Admin\
```
...
- `require.php`: already `~8.4.1 || ~8.5.0` (no `8.6.0-dev`). Add
  `config.platform.php: 8.4.99`.
- `require-dev`: PHPUnit already `^13.3`; add Infection `^0.34.1`, PHPBench `^1.7`,
  roave/backward-compatibility-check `^8.21.0`, `roave/security-advisories`; keep
  `webware/coding-standard`, `webware/webware-tools`.
- `autoload-dev`: rename `Webware\AdminTest\` → `WebwareTest\Admin\` (test/unit) and
  `Webware\AdminIntegrationTest\` → `WebwareTestIntegration\Admin\` (test/integration); update
  test file namespaces to match.
- `scripts`:
...
- `run-integration` = `false` in CI until the integration suite exists; the `integration test`
  suite still gets one scaffolding test to satisfy FR-012.
...
- `.github/workflows/continuous-integration.yml`: wrapper with package inputs:
